<template>
	<div class="tables-basic">
		<b-breadcrumb>
			<b-breadcrumb-item>YOU ARE HEAR</b-breadcrumb-item>
			<b-breadcrumb-item>Tables Basic</b-breadcrumb-item>
		</b-breadcrumb>
		<h2 class="page-title">
	...
	export default {
		...
		methods: {
			...
			CheckAll(ev, checkbox) {
				...
				this.$set(this, checkbox, checkboxArr);
			},
			changeCheck(ev, checkbox, id) {
				...
				if (!ev.target.checked) {
					this.$set(this[checkbox], 0, false);
				} else {
          this.$set(this[checkbox], 0, true);
        }
			},
			getRandomData() {
				const res = [];
				for (let i = 0; i <= 8; i += 1) {
					res.push(Math.floor(Math.random() * 20) + 1);
				}
				return [{ data: res }];
			},
			getRandomColor() {
